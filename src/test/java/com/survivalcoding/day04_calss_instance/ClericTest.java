...
class ClericTest {
    ...
    void testConstructorOverloading() {
        ...
        // 이름만 입력한 경우
        assertEquals("Arthur", nameOnly.getName());
        assertEquals(Cleric.MAX_HP, nameOnly.getHp());
        assertEquals(Cleric.MAX_MP, nameOnly.getMp());
        
        // 이름과 HP만 입력한 경우
        assertEquals("Uther", nameAndHp.getName());
        assertEquals(35, nameAndHp.getHp());
        assertEquals(Cleric.MAX_MP, nameAndHp.getMp());
    }
    ...
    void selfAidTest() {
        ...
        cleric.setHp(35);
        cleric.setMp(MP_COST - 1);
        cleric.selfAid();
        assertEquals(MP_COST - 1, cleric.getMp());
        ...
        /* 기존 4: MP가 넉넉한 경우 */
        cleric.setHp(35);
        cleric.setMp(MP_COST + 3);
        cleric.selfAid();
        assertEquals(3, cleric.getMp());
        assertEquals(Cleric.MAX_HP, cleric.getHp());
        
        /* 추가: 이미 HP가 MAX일 때 사용 (MP만 소모) */
        cleric.setHp(Cleric.MAX_HP);
        cleric.setMp(Cleric.MAX_MP);
        cleric.selfAid();
        assertEquals(Cleric.MAX_HP, cleric.getHp());
        assertEquals(Cleric.MAX_MP - MP_COST, cleric.getMp());
    }
    ...
    void prayTest() {
        ...
        assertEquals(randomRecovery, cleric.getMp());

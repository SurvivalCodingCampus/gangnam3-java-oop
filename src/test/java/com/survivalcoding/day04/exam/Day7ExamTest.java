...
public class Day7ExamTest {
    ...
    class PoisonSlimeTest {
        ...
        void 독카운트는_0이상이면_정상(int count) {
            PoisonSlime poisonSlime = new PoisonSlime("A", count);
        ...
        void 독카운트는_0미만이면_예외(int invalidCount) {
            ...
            assertThatThrownBy(() -> new PoisonSlime("A", invalidCount))
    ...
    class SuperHeroTeest {
        ...
        void 비행상태아니면_기본공격만() {
            ...
            SuperHero superHero = new SuperHero("영웅", Hero.MAX_HP);
            Slime slime = new Slime("A");
            int beforeSlimeHp = slime.getHp();

            superHero.attack(slime, superHero.power);

            assertThat(slime.getHp()).isEqualTo(beforeSlimeHp - superHero.power);
        }
        ...
        void 비행상태면_기본공격과_추가공격() {
            ...
            assertThat(slime.getHp()).isEqualTo(Math.max(0, beforeSlimeHp - (superHero.power + SuperHero.BONUS_DAMAGE)));
        }
    }
    ...
    class WizardTest {
        ...
        void 힐하면_히어로체력을_20힐하고_위자드Mp를_10감소한다() {
            ...
            wizard.heal(hero, Wizard.COST_FOR_HEAL, Wizard.HEAL_HP_AMOUNT, Wizard.HEAL_SKILL_NAME);
